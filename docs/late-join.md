...
transaction**. The 2026-06-09 brookr rehearsal followed exactly this path.

## What you end up with

- Your `node` branch carries every archive day (verified upstream adoptions
  plus your own captures), all with v2 content fields.
...
1. **Hydrates** every day before `keep_from` from the upstream node's
   published v2 bundles — fetching the upstream storage receipt, verifying the
...
2. **Rebuilds** v2 content fields for your own days that lack them
   (idempotent; adopted days are skipped so their artifacts stay
   byte-identical to the published bundles).
3. **Publishes** your own days' v2 bundles to your releases (adopted days are
...
4. **Signs** every day in the range with your repo-secret signer. Parent
   hashes chain from the v2 genesis (whose parent is the agreed v1 head
   block); your locator carries your own `nodeId` with the verified bundle
   fingerprint and locations.
...
export RSO_DOCCHAIN_ADDRESS=0x2c66585E7b60A20563a3fd2B7a4D75Ae5baa5437
...
Space-Track (see `docs/chain-v2.md`, "Verifying the chain from nothing") and
